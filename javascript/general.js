...
function change_page_content(response) {
    $('.right-content').promise().done(function(){
        ...
        jQuery('div.right-content h2.title').html(response.data.title);
        jQuery('div.right-content div.wrapper div').html(response.data.right_content);
        ...
        jQuery('.right-content').animate({
                ...
                jQuery(this).animate({
                    ...
                    var prev_active = jQuery('ul.menu li.active').index();
                    var new_active = jQuery('ul.menu li#' + g_current_active_li).index();
                    ...
                    if(new_active - prev_active > 0){
                        for(i = prev_active+1; i <= new_active + 1; i++){
                            (function(li){
                                setTimeout(function(){
                                    jQuery('ul.menu li:nth-child(' +li+ ')').addClass('active');
                                },100*counter);

                                if(i != new_active +1){
                                    setTimeout(function(){
                                        jQuery('ul.menu li:nth-child(' +li+ ')').removeClass('active');
                                    },100* (counter + len));
                                }
                            }(i));
